...
header_template = Waterfall(wav, FFT_SIZE, FFT_SHIFT);
...
% header_template = header_template - mean(mean(header_template));
...
for i = 1 : search_range(1)
  for j = 1 : search_range(2)
    box = wfnorm(i : i + template_size(1) - 1, j : j + template_size(2) - 1);
